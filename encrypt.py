...
class URLParseHelper:
    ...
    def ssrObj(self):
        ...
        if self.url.find('_')>0:
            ...
            params = { k: strDecode(",".join(v) if v else v) for k,v in params.items()}
        else:
            params = {}
        params['remarks'] = _tagName
        
        params = [ f'{k}={strEncode(v)}' for k,v in params.items()]
        params = "&".join(params)
        ...
        if _url_qs:
            ...
            _newUrl = self.urlObj.scheme + '://' + b + '_' + strEncode(params)
        else:
...
if __name__ == "__main__":
    ...
    match sys.argv[1]:
        ...
        case 'debug':
            url = "ssr://MTI3LjAuMC4xOjEwMDg2Om9yaWdpbjpjaGFjaGEyMC1pZXRmOnBsYWluOmVBLz9ncm91cD1ZV2x5JnJlbWFya3M9OEotSHVQQ2ZoNndnNHBxZzc3aVA1cHUwNXBhdzZLNmk2WmlGVy1XbWd1YWVuT2lLZ3VlQ3VlZVVxT1M0amVTNmhsMA"
            print(url)
